...
public abstract class Animation {
  ...
  /**
   * The unique ID of the current run. Used to handle cases where an animation is restarted within
   * an execution block.
   */
  private int runId = -1;
  ...
  /**
   * Construct a new {@link AnimationScheduler} using the specified scheduler to sheduler request
   * frames.
   *
   ...
   * Immediately run this animation. If the animation is already running, it will be canceled first.
   *
   * <p>If the element is not <code>null</code>, the {@link #onUpdate(double)} method might be
   * called only if the element may be visible (generally left at the appreciation of the browser).
   * Otherwise, it will be called unconditionally.
   *
   * @param duration the duration of the animation in milliseconds
  ...
  /**
   * Run this animation at the given startTime. If the startTime has already passed, the animation
   * will run synchronously as if it started at the specified start time. If the animation is
   * already running, it will be canceled first.
   *
   * <p>If the element is not <code>null</code>, the {@link #onUpdate(double)} method might be
   * called only if the element may be visible (generally left at the appreciation of the browser).
   * Otherwise, it will be called unconditionally.
  ...
  /**
   * Immediately cancel this animation. If the animation is running or is scheduled to run, {@link
  ...
  /**
   * Called immediately after the animation is canceled. The default implementation of this method
   * calls {@link #onComplete()} only if the animation has actually started running.
   */
   ...
   *
   * <p>The value of progress is between 0.0 and 1.0 (inclusive) (unless you override the {@link
   * #interpolate(double)} method to provide a wider range of values). There is no guarantee that
   * {@link #onUpdate(double)} is called with 0.0 or 1.0. If you need to perform setup or tear down
   * procedures, you can override {@link #onStart()} and {@link #onComplete()}.
   *
   ...
   *
   * <p>Depending on the {@link Animation}, the return value of this method can be less than 0.0 or
   * greater than 1.0.
   *
   ...
   * already running, it will be canceled first.
   *
   * <p>This is equivalent to <code>run(duration, startTime, null)</code>.
   *
   * @param duration the duration of the animation in milliseconds
   * @param startTime the synchronized start time in milliseconds
